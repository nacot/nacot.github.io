...
ringCanvas.addEventListener('mousemove', function(evt) {
    pmouseX = mouseX;
    pmouseY = mouseY;
    mousePos = getMousePos(ringCanvas, evt);
    mouseX = mousePos.x;
    mouseY = mousePos.y;
...
ringCanvas.addEventListener('mousedown', function(evt) {
    mousePressed = true;
    //clog(mousePressed + " at " + mouseX +", "+mouseY);
}, false);

ringCanvas.addEventListener('mouseup', function(evt) {
    mousePressed = false;
}, false);

ringCanvas.addEventListener('mouseout', function(evt) {
    mousePressed = false;
}, false);
...
function mouseDraggingCheck() {
    if (mousePressed  &&  testCoordinatesInside(mouseX, mouseY, 100, 100, camWidth-100, camHeight-100 ) ) {
...
clickAreas[clickAreas.length] = new RectCoordinates(100,100,camWidth-100,camHeight-100);  //ROTATE VIEW ZONE
/*
...
function setCursorToHand(){
    ringCanvas.style.cursor = "hand";
}

function setCursorToNormal(){
    ringCanvas.style.cursor = "default";
}

document.getElementById( "ring_canvas" ).onmousedown = function(event){
