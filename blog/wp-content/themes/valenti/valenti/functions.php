...
add_image_size('cb-80-60', 80, 60, true); // Used on sidebar widgets
...
add_image_size('cb-600-250', 600, 250, true ); // Used on grid 4, grid 6
add_image_size('cb-600-400', 600, 400, true ); // Used on grid 4, 5, 6, 7
...
if ( ! function_exists( 'cb_scripts_and_styles' ) ) {   
    function cb_scripts_and_styles() {        
      if (!is_admin()) {
        ...
        if ( is_rtl() ) {
            wp_register_style( 'cb-main-stylesheet',  get_template_directory_uri() . '/library/css/style-rtl.css', array(), '1.0', 'all' );
        } else {
        ...
        // Register fonts
        $cb_font = cb_fonts();
        wp_register_style( 'cb-font-stylesheet',  $cb_font[0], array(), '1.0', 'all' );
        wp_enqueue_style('cb-font-stylesheet'); // enqueue it
        wp_register_style( 'cb-font-body-stylesheet',  $cb_font[1], array(), '1.0', 'all' );
        wp_enqueue_style('cb-font-body-stylesheet'); // enqueue it
...
if ( ! function_exists( 'cb_post_admin_scripts_and_styles' ) ) {
    function cb_post_admin_scripts_and_styles($hook) {
    	...
    	if( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'profile.php' || $hook == 'appearance_page_ot-theme-options' || $hook == 'user-edit.php') {
