...
function waveformTimeline( waveformScale, tempDrawHeight ) {
    ...
    context2d.lineWidth = 1/widthMult*2;
    waveformValues = smoothedPoints;

    //DRAW GRAPH
    context2d.setTransform(1,0,0,1,0,0);
    context2d.translate( 0, tempDrawHeight, 0 );
    for (var i = 0; i < smoothedPoints.length; i++) {
        context2d.strokeStyle=colorWhite;
        context2d.beginPath();
        context2d.moveTo(i/widthMult, 0);
        context2d.lineTo( i/widthMult, -constrain( smoothedPoints[i]*0.5, 0.0, 70.0 ) );
        context2d.stroke();
    }
    context2d.closePath();
    context2d.setTransform(1,0,0,1,0,0);
...
function drawLiveCreationHelpers() {
    ...
    context2d.setTransform(1,0,0,1,0,0);
    context2d.translate(0, 60, 0);
    context2d.beginPath();
    context2d.moveTo( i, lastBuffer[0]*waveformBufferHeight );
    var maxValue = 0;
    for ( var i = 0; i < lastBuffer.length-1; i++) {
        context2d.lineTo( (i+1), lastBuffer[i+1]*waveformBufferHeight*normalizerMult );
...
function calcRevolvedPoints() {
    ...
    for (var i = 0; i<ringPathRes; i++) {
        for(var j=0; j<ringSectionRes; j++){
            ...
            var iStore = i;
            i = ( i + sectionRotation )%ringPathRes;

            temp =
                pointsLoftSection[0].points[j] * loftSectionMultipliers[i][0] * ringAsymmetry[0] +
                pointsLoftSection[1].points[j] * loftSectionMultipliers[i][1] * ringAsymmetry[1] +
                pointsLoftSection[2].points[j] * loftSectionMultipliers[i][2] * ringAsymmetry[2] +
                pointsLoftSection[0].points[j] * loftSectionMultipliers[i][3] * ringAsymmetry[0] ;
            temp = temp*tempC*tempB*amplitude + sectionRadius;

            if(index<0) clog(index);
...
function volumeOfMesh(mesh) {
    ...
    //for( var t in mesh.faces ){
    for( var i=0; i<mesh.faces.length; i++){
        var t = mesh.faces[i];
        volSum += signedVolumeOfTriangle( points[t.a], points[t.b], points[t.c] );
    }

    return Math.abs( volSum );
}

function calcRingVolume(){
    ringVolume = Math.pow(exportSizeNormalizer*exportRingSize, 3) * volumeOfMesh( ringDisplay.geometry );
    return ringVolume;
}
