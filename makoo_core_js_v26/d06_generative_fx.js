...
function waveformToPoints(){
    ...
    if(maxDropCrossingRatio>0.8 && !recording){
        autoSmoothRounds++;
        autoSmoothMult *= 1.05;
        if(autoSmoothRounds > 5) autoSmoothMult *= 1.2;
        autoAmplitudeMult *= 1;
        ...
        if(autoSmoothRounds < 10) waveformToPoints();
    }
}
var autoSmoothRounds = 0;
...
function rippleCircle(strength){
    this.strength = Math.pow( strength, 0.25 );
    this.radius = 20;
    this.active = true;
    this.age = 0;
    //clog(this.strength);
    ...
    this.update = function(){
        if(this.active){
            ...
            if(this.age<10) temp = this.age/10;
                ...
                if(this.age>20) temp = 1 - (this.age-20)/50;
                else temp = 1;
            }
            temp *= 0.8;
...
for(var i=0; i<ripples.length; i++){
    ripples[i] = new rippleCircle();
    ripples[i].active = false;
}
var rippleToUpdate = 0;
var rippleStartBuffer = 0;
var rippleTreshold = 20000;
...
function drawLiveCreationHelpers() {
    ...
    recordContext.translate(300, 100, 0);
...
function setupRingGeometry() {
    ...
    */
    ringDisplayDouble = THREE.SceneUtils.createMultiMaterialObject( ringDisplay.geometry, multiMaterial );
    scene.add(ringDisplayDouble);
    ringDisplayDouble.scale.x = 0.6;
    ringDisplayDouble.scale.y = 0.6;
    ringDisplayDouble.scale.z = 0.6;
    ringDisplayDouble.rotation.x = 0.0;
